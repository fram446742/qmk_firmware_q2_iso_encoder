 ...
 *
 * NOTE: Adapted for upstream QMK — wired-only (src always USB 0).
 */
...
void kc_raw_hid_send(uint8_t *data, uint8_t len) {
...
#    if defined(KEYCHRON_RGB_ENABLE)
        case 0xA8: {
            ...
            kc_rgb_matrix_rx(true, data, length);
        } break;
#    endif
...
#    ifdef FACTORY_TEST_ENABLE
        case 0xAB:
            factory_test_rx(true, data, length);
            return true;
#    endif
        default:
            return false;
...
#    else
/* Without VIA, override raw_hid_receive directly */
